...
"""
This is a util file that adds a pretty-printing styling to absl.logging calls.  
In addition to making the logs a bit nicer it will add slurm info to the log.
Be careful as it can mess up the order of prints and stuff, its not the best code


It can be used with either a:

 `from standard_logger import logging`
...
and then using the standard logging calls e.g `logging.info(...)`
"""
...
class LogFormatter(py_logging.Formatter):
    """
